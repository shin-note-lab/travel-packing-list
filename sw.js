const CACHE_NAME = "travel-packing-v10.22.3";
const APP_SHELL = [
  "./",
  "./index.html",
...
self.addEventListener("activate", event => {
  event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(key => key.startsWith("travel-packing-") && key !== CACHE_NAME).map(key => caches.delete(key)))).then(() => self.clients.claim()));
});
